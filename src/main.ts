...
class GitHub {
	...
	getTree(branchName: string): string {
		const resp = this.doRequest(
			`${this.baseURL}/git/trees/${branchName}`,
			"get",
			{},
		);
		return resp.sha;
	...
	createBranch(newBranchName: string, baseSha: string): string {
		const resp = this.doRequest<{ object: Response }>(
...
function call() {
	...
	const g = new GitHub(pat, username, repo);
	g.doSimpleRequest(
		`${g.baseURL}/actions/workflows/json.yml/dispatches`,
		"post",
		{ ref: "main", inputs: { json } },
	);
}
